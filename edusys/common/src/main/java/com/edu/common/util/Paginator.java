...
public class Paginator {
    ...
    public String getHtml() {
        ...
        // 结果html
        String pages = "<ul class=\"pagination pagination-lg\">";
        ...
        if (page > 1) {
            if (url.contains("?")) {
                pages = pages.concat("<li><a href=\"" + url + "&" + param + "=" + (page - 1) + "\">«</a></li>");
            } else {
                pages = pages.concat("<li><a href=\"" + url + "?" + param + "=" + (page - 1) + "\">«</a></li>");
            }
        } else {
            // 特定需求可隐藏
            pages = pages.concat("<li class=\"disabled\"><a href=\"javascript:;\">«</a></li>");
        }
        // 如果总页数大于要显示的个数，则拼接显示
        if (pageCount > step) {
            // 显示分页码
            ...
            if (listBegin >= 2) {
                if (url.contains("?")) {
                    pages = pages.concat("<li><a href=\"" + url + "&" + param + "=1\">1</a></li>");
                } else {
                    pages = pages.concat("<li><a href=\"" + url + "?" + param + "=1\">1</a></li>");
                }
            }
            ...
            if (pageCount - page >= page - listBegin) {
                for (int i = listBegin; i < (listBegin + step); i++) {
                    if (i != page) {
                        if (url.contains("?")) {
                            pages = pages.concat("<li><a href=\"" + url + "&" + param + "=" + i + "\">"+i+"</a></li>");
                        } else {
                            pages = pages.concat("<li><a href=\"" + url + "?" + param + "=" + i + "\">"+i+"</a></li>");
                        }
                    } else {
                        pages = pages.concat("<li class=\"active\"><a href=\"javascript:void(0);\">"+i+"</a></li>");
                    }
                }
                // 显示最后1页
                if (listBegin + step <= pageCount) {
                    if (url.contains("?")) {
                        pages = pages.concat(" ... <li><a href=\"" + url + "&" + param + "=" + pageCount + "\">"+pageCount+"</a></li>");
                    } else {
                        pages = pages.concat(" ... <li><a href=\"" + url + "?" + param + "=" + pageCount + "\">"+pageCount+"</a></li>");
                    }
                }
