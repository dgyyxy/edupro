		...
		<div class="form-group">
			<select id="roleId" name="roleId" multiple="multiple" style="width: 100%">
				<c:forEach var="role" items="${roles}">
					<option value="${role.roleId}" <c:forEach var="userRole" items="${userRoles}"><c:if test="${role.roleId==userRole.roleId}">selected="selected"</c:if></c:forEach>>${role.title}</option>
				</c:forEach>
			</select>
		</div>
